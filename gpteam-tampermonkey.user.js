        ...
        async handleOAuth(email) {
            try {
                ...
                const confirmed = await this.showCustomConfirm(
                    '即将为以下邮箱进行 OAuth 授权',
                    `邮箱：${email}\n\n` +
                    `授权完成后，浏览器会跳转到 localhost:1455（无法访问）。\n` +
                    `请复制完整的回调 URL 并粘贴到下一步的输入框中。`
                );
                ...
                const callbackUrl = await this.showCustomPrompt(
                    '粘贴回调 URL',
                    '请复制浏览器地址栏的完整 URL 并粘贴到下方：\n\n（格式：http://localhost:1455/auth/callback?code=...&state=...）',
                    ''
                );

                if (!callbackUrl) {
                    this.showCustomAlert('已取消授权', 'info');
                    return;
                }
                ...
                try {
                    ...
                    // 显示加载中
                    const loadingOverlay = this.showLoading('正在交换 Token...');

                    // 交换Token
                    console.log('开始交换Token...');
                    codexOAuth.exchangeToken(code, codeVerifier, email)
                        .then(result => {
                            // 保存Token
                            if (loadingOverlay) {
                        ...
                        .catch(error => {
                            ...
                            this.showCustomAlert(`Token交换失败：\n\n${error.message}`, 'error');
                        });

                } catch (error) {
                    console.error('解析回调URL失败:', error);
                    this.showCustomAlert(`解析回调URL失败：\n\n${error.message}`, 'error');
                }
